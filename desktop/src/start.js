...
ipcMain.on('delete-brewery', function (event, item) {
  console.log('delete-brewery');
  callDbAndSendResponseEvent(database.deleteBrewery,item);
})

let count = 0;
if(count===0){
  const data = require('../brew');
  console.log(data);
  for(var item in data){
console.log("ITM: *****",data[item]);
    database.insertBreweryFromUi(data[item]);
  }
 count= 1;
}